...
public abstract class HornetQBroker implements Broker {
	
	private static final DebugLogger logger = DebugLogger
			.getLogger("hqbroker.log");

	private static final Logger LOG = LoggerFactory
			.getLogger(HornetQBroker.class);
	...
	public HornetQBroker() {
		try {
			...
			logger.log("temp dir folder created: " + temporaryFolder.getRoot());
		} catch (IOException e) {
			logger.log("failed to create temp dir", false, e);
		}
	}
	@Override
	...
	protected Configuration createDefaultConfig(
		...
		ConfigurationImpl configuration = createBasicConfig(-1)
				.setFileDeploymentEnabled(false).setJMXManagementEnabled(false)
